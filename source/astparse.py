...
class Import:
    __slots__ = ('module', 'symbols', 'src_file', '_token')
...
def parse_expr_stmt(tokens: list[Token], pos: int):
    ...
    if pos < len(tokens) and tokens[pos].type in compound_assign_ops:
        if not _is_assignable(expr):
            raise ParseError('Invalid assignment target', tokens[pos])
        target = expr
        op = compound_assign_ops[tokens[pos].type]
        pos += 1
        value, pos = parse_expression(tokens, pos)
        _expect_newline(tokens, pos, tok)
        return Assign(target, BinOp(Variable(target.name, token=expr._token) if isinstance(target, Variable) else target, getattr(TokenType, tokens[pos - 1].type.name.replace('_EQ', ''), TokenType.PLUS), value, token=tok), token=tok), pos
