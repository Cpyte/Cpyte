...
def _eval_constant(code: str):
    ...
    for op, fn in [
        ...
        (
            ...
            lambda a, b: (
                a << b
                if isinstance(a, int) and isinstance(b, int) and b >= 0 and b < 128
                else None
            ),
        ),
        (
            ">>",
            lambda a, b: (
                a >> b if isinstance(a, int) and isinstance(b, int) and b >= 0 else None
            ),
        ),
        ("+", lambda a, b: a + b),
        ("-", lambda a, b: a - b),
...
def expr_is_always_truthy(
    code: str, known_truthy_globals: set[str] | None = None
) -> bool:
...
def _gen_int_literal(rng: random.Random, target: str) -> str:
    ...
    if target == "int64":
        if rng.random() < 0.35:
            return rng.choice(
                ["2147483648", "4294967296", "1000000000000", "9223372036854775807"]
            )
        return str(rng.randint(2**31, 2**63 - 1))
    if target == "uint64":
        if rng.random() < 0.35:
            return rng.choice(
                ["9223372036854775808", "10000000000000000000", "18446744073709551615"]
            )
    ...
    if rng.random() < 0.35:
        return rng.choice(
            ["0", "1", "2", "5", "42", "-1", "-5", "100", "1000", "1000000"]
        )
...
def gen_variable(
    state: FuzzerState, target_type: str | None = None
) -> tuple[str, str] | None:
    candidates = [
        (n, t) for n, t in state.scope.all_vars() if _assignable(t, target_type)
    ]
    if not candidates:
        return None
...
def _ensure_nonzero(
    state: FuzzerState, value: tuple[str, str], depth: int, exact: frozenset | None
) -> tuple[str, str]:
...
def gen_expr(
    ...
    want_num = (
        target_type is None or target_type in INT_FAMILY or target_type in FLOAT_FAMILY
    )
    want_ptr = target_type is None or type_is_ptr(target_type)
    want_scalar = (
        target_type is None
        or target_type in SCALAR_GEN_TYPES
        or type_is_ptr(target_type)
    )
    ...
    if want_scalar:
        candidates.append(
            ("field", lambda: gen_field_access(state, target_type, depth))
        )
        weights.append(5)
...
def gen_binop(
    state: FuzzerState, target_type: str | None, depth: int
) -> tuple[str, str]:
    rng = state.rng

    if target_type is None:
        cat = rng.choices(
            ["arith_int", "arith_float", "bitwise", "mod", "shift"],
        ...
        cat = rng.choices(
            ["arith_int", "bitwise", "mod", "shift"], weights=[45, 15, 20, 20]
        )[0]
    ...
    if cat == "mod":
        op = rng.choice(["//", "%"])
        left = gen_expr(
            state, rng.choice(["int", "int64", "uint64", "big"]), depth + 1, MOD_OK
        )
        right = gen_expr(
            state, rng.choice(["int", "int64", "uint64", "big"]), depth + 1, MOD_OK
        )
    ...
    if cat == "shift":
        op = rng.choice(["<<", ">>"])
        left = gen_expr(
            state, rng.choice(["int", "int64", "uint64"]), depth + 1, BITWISE_OK
        )
        right = gen_expr(state, "int", depth + 1, BITWISE_OK)
        return f"({left[0]} {op} {right[0]})", (
            "int64" if left[1] in ("int64", "uint64") else "int"
        )
    ...
    result = (
        "int64"
        if left[1] in ("int64", "uint64") or right[1] in ("int64", "uint64")
        else "int"
    )
...
def gen_unary(
    ...
    if target_type is None:
        kind = rng.choices(
            ["neg", "not", "tilde", "deref", "addr"], weights=[25, 25, 15, 20, 15]
        )[0]
    elif target_type in ("float", "double"):
        kind = "neg"
    ...
    if kind == "neg":
        op_t = (
            target_type
            if target_type in NEG_OK
            else rng.choice(["int", "int64", "uint64", "big", "float", "double"])
        )
...
def gen_field_access(
    ...
    struct_vars = [
        (n, t)
        for n, t in state.scope.all_vars()
...
def gen_index_expr(
    ...
    rng = state.rng
    arr_vars = [
        (n, t) for n, t in state.scope.all_vars() if t.endswith("*") or t.endswith("[]")
    ]
    if not arr_vars:
        return gen_literal(state, target_type)
...
def gen_compare(state: FuzzerState, depth: int) -> tuple[str, str]:
    ...
    if fam == "int":
        ...
        right = gen_expr(
            state, rng.choice(["float", "double"]), depth + 1, FLOAT_FAMILY
        )
    else:
...
def gen_if(state: FuzzerState, indent: int, depth: int) -> str:
    ...
    if rng.random() < 0.3:
        else_body = gen_body_in_scope(state, depth + 1, indent + 1) or [
            f"{pad}    print(0)"
        ]
        result += f"\n{pad}else:\n" + "\n".join(else_body)
    return result
...
def gen_func_def(state: FuzzerState, indent: int = 0) -> str:
    ...
    ret_type = rng.choices(
        [None] + TYPES + ["void"], weights=[30, 10, 10, 10, 5, 5, 5, 5, 10]
    )[0]
...
def run_test(source, label=""):
    ...
    if TEST_COUNT % 500 == 0:
        ...
        p = PASS_COUNT
        print(
            f"  [{n}] pass={p} reject={r} crashes={CRASH_COUNT} bugs={BUG_COUNT}",
            flush=True,
        )
