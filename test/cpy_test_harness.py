...
def gen_literal(state, target_type=None):
    ...
    if target_type == "char":
        return (
            repr(rng.choice(string.ascii_letters + string.digits + " \t\n")),
            "char",
        )
    if target_type == "big":
    ...
    if target_type == "str":
        ...
        s = "".join(
            rng.choices(string.ascii_lowercase + string.digits + "\t\n", k=length)
        )
        return (repr(s), "str")
    ...
    if state.scope.all_vars():
        ...
        choices.append(
            (
                10,
                lambda: (
                    gen_variable(state, rng.choice(NUMERIC_TYPES)) or gen_literal(state)
                ),
            )
        )
    if depth < max_depth:
...
def run_program(source, label="", save=True):
    ...
    if err0 is None and (out0 != out3 or ret0 != ret3):
        _save_crash(
            source, f"output mismatch: unopt={out0!r}/{ret0} opt={out3!r}/{ret3}"
        )
        return "bug"
    return "pass"
...
def _mutate_ast_node(node, rng):
    ...
    if isinstance(node, VarDecl):
        if node.init is not None:
            return VarDecl(
                node.var_type, node.name, _mutate_ast_node(node.init, rng), node._token
            )
        return node
    ...
    if isinstance(node, NewExpr):
        if node.size is not None:
            return NewExpr(
                node.type_expr, _mutate_ast_node(node.size, rng), node._token
            )
        return node
    ...
    if isinstance(node, Index):
        return Index(
            _mutate_ast_node(node.value, rng),
            _mutate_ast_node(node.index, rng),
            node._token,
        )
...
def _ast_to_source(node, indent=0):
    ...
    if isinstance(node, FuncDef):
        ...
        body = "\n".join(
            f"    {_ast_to_source(s, indent + 1)}" for s in (node.body or [])
        )
        return f"def {node.name}({params}) -> {node.return_type}:\n{body}"
    if isinstance(node, StructDef):
        fields = "\n".join(
            f"    {_ast_to_source(f, indent + 1)}" for f in (node.fields or [])
        )
    ...
    if isinstance(node, If):
        ...
        if hasattr(node, "orelse") and node.orelse:
            else_body = "\nelse:\n" + "\n".join(
                f"    {_ast_to_source(s, indent + 1)}" for s in node.orelse
            )
    ...
    for i in range(n_iter):
        ...
        if rng.random() < 0.6:
            ...
            if len(var_names) >= 2:
                ...
                mutated = (
                    mutated.replace(a, "__TMP__").replace(b, a).replace("__TMP__", b)
                )
        ...
        if (i + 1) % 200 == 0:
            print(
                f"  mutate [{i + 1}/{n_iter}] pass={stats['pass']} "
                f"reject={stats['reject']} bug={stats['bug']}",
                flush=True,
            )
...
def run_fuzz(n=5000, seed=None):
    ...
    for i in range(n):
        ...
        if (i + 1) % 500 == 0:
            print(
                f"  [{i + 1}/{n}] pass={stats['pass']} reject={stats['reject']} "
