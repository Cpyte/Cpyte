...
def run_jit(module, opt_level=3, src_files=None):
    ...
    _print_fn = ctypes.CFUNCTYPE(None, ctypes.c_int)(_runtime_print)
    try:
        engine.add_global_mapping(
            mod.get_function("print_int"),
            ctypes.cast(_print_fn, ctypes.c_void_p).value,
        )
    except NameError:
        pass

    _print_int64_fn = ctypes.CFUNCTYPE(None, ctypes.c_longlong)(_runtime_print_int64)
    try:
        engine.add_global_mapping(
    ...
    _print_double_fn = ctypes.CFUNCTYPE(None, ctypes.c_double)(_runtime_print_double)
    try:
        engine.add_global_mapping(
            mod.get_function("print_double"),
            ctypes.cast(_print_double_fn, ctypes.c_void_p).value,
        )
    except NameError:
        pass

    _input_fn = ctypes.CFUNCTYPE(ctypes.c_int)(_runtime_input)
    try:
        engine.add_global_mapping(
            mod.get_function("input_int"),
            ctypes.cast(_input_fn, ctypes.c_void_p).value,
        )
    except NameError:
        pass

    _print_str_fn = ctypes.CFUNCTYPE(None, ctypes.c_char_p)(_runtime_print_str)
    try:
        engine.add_global_mapping(
            mod.get_function("print_str"),
            ctypes.cast(_print_str_fn, ctypes.c_void_p).value,
        )
    except NameError:
        pass
    ...
    ctypes.CFUNCTYPE(ctypes.c_int)(func_ptr)()
