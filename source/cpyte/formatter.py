...
class _Formatter:
    ...
    def _unary_text(self, node):
        ...
        if isinstance(operand, astparse.BinOp):
            ...
            operand_text = self._expr(operand)
        if (
            node.op in (TokenType.MINUS, TokenType.PLUS, TokenType.MINUS_MINUS)
            and isinstance(operand, astparse.UnaryOp)
            and operand.op in (TokenType.MINUS, TokenType.PLUS, TokenType.MINUS_MINUS)
        ):
    ...
    def _postfix_base(self, node):
        if isinstance(
            node, (astparse.BinOp, astparse.UnaryOp, astparse.Deref, astparse.AddrOf)
        ):
    ...
    def _emit_assign(self, node, level: int):
        ...
        if isinstance(value, astparse.BinOp):
            left = value.left
            if left is node.target or (
                isinstance(left, astparse.Variable)
                and isinstance(node.target, astparse.Variable)
                and left._token is node.target._token
            ):
    ...
    def _emit_tail(self, orelse, level: int):
        ...
        if (
            len(orelse) == 1
            and isinstance(orelse[0], astparse.If)
            and getattr(orelse[0]._token, "value", None) == "elif"
        ):
    ...
    def _emit_for(self, node, level: int):
        ...
        self._flush_before(_line(node), level)
        self._out.append(
            self._indent(level) + f"for {node['var']} in {self._expr(node['iter'])}:"
        )
        if tok is not None:
    ...
    def _emit_funcdef(self, node, level: int):
        ...
        params = ", ".join(
            f"({n}): {t}" if n in const_params else f"{n}: {t}"
            for n, t in node.params.items()
        )
    ...
    def _emit_raise(self, node, level: int):
        self._emit_simple(
            node, f"raise {node.exc_type}({self._expr(node.message)})", level
        )
...
def _need_parens(child, parent_op, is_right: bool) -> bool:
    if not isinstance(child, astparse.BinOp):
        return False
    parent_p = _op_prec(parent_op)
    child_p = _op_prec(child.op)
    if is_right:
...
def _op_prec(op) -> int:
    if op == TokenType.KEYWORD:
        return astparse.IN_PREC
    return astparse._PREC.get(op, 0)
...
def format_source(
    source: str, tab_size: int = 4, enable_extensions: bool = True
) -> FormatResult:
    ...
    result = FormatResult(
        formatted=text, errors=fmt.errors, comment_count=len(comments), parsed=parsed
    )
    if not fmt.errors:
        re_comment_count = len(_scan_comments(text))
        if re_comment_count != len(comments):
            result.errors.append(
                f"comment preservation failed: {len(comments)} -> {re_comment_count}"
            )
    return result
...
def format_file(
    ...
    result = format_source(
        source, tab_size=tab_size, enable_extensions=enable_extensions
    )
    if check:
        if result.errors:
            return result, 1
